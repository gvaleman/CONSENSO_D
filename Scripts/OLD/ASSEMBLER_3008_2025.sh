...
# Detectar la ubicación del script clasificador
# Se corrige la lógica para buscar el script dentro del directorio CONSENSO_D
...
elif [ -f "$current_dir/CONSENSO_D/DENV_Universal_Classifier.py" ]; then
    classifier_script="$current_dir/CONSENSO_D/DENV_Universal_Classifier.py"
...
elif [ -f "$current_dir/DENV_Universal_Classifier.py" ]; then
    classifier_script="$current_dir/DENV_Universal_Classifier.py"
...
run_batch_classification() {
    ...
    if [ "$classification_available" = true ]; then
        ...
        # Eliminar lógica manual de búsqueda y preparación de archivos.
        # El clasificador de Python lo hará automáticamente con --search-dir.
...
# Generar resumen de clasificaciones si están disponibles
if [ "$classification_available" = true ] && [ -d "Lineage_Classification" ]; then
    echo ""
    echo "🧬 GENERANDO RESUMEN FINAL DE CLASIFICACIONES..."
    
    # Buscar archivos de clasificación
    classification_files=($(find Lineage_Classification -name "*_classification*.csv" -type f))
    
    if [ ${#classification_files[@]} -gt 0 ]; then
        # Buscar el archivo batch principal
        batch_file="Lineage_Classification/batch_${serotype}_classifications.csv"
        
        if [ -f "$batch_file" ]; then
            echo ""
            echo "📊 RESUMEN FINAL DE LINAJES IDENTIFICADOS:"
            echo "============================================"
            
            # Estadísticas generales
            total_samples=$(tail -n +2 "$batch_file" | wc -l)
            echo "    📝 Total de muestras procesadas: $total_samples"
            echo "    🦠 Serotipo analizado: $serotype"
            ...
            if [ $total_samples -gt 0 ]; then
                ...
                echo "🧬 DISTRIBUCIÓN POR LINAJES:"
                echo "----------------------------"
                tail -n +2 "$batch_file" | cut -d',' -f3 | sort | uniq -c | sort -nr | while read count lineage; do
                    percentage=$(echo "scale=1; ($count * 100) / $total_samples" | bc)
                    echo "    $lineage: $count muestra(s) ($percentage%)"
                done
                ...
                echo "📈 DISTRIBUCIÓN POR CONFIANZA:"
                echo "------------------------------"
                tail -n +2 "$batch_file" | cut -d',' -f4 | sort | uniq -c | sort -nr | while read count confidence; do
                    echo "    Confianza $confidence: $count muestra(s) ($percentage%)"
                done
                
                # Muestras ambiguas
                ambiguous_count=$(tail -n +2 "$batch_file" | awk -F',' '$8 == "True" || $8 == "true"' | wc -l)
                if [ $ambiguous_count -gt 0 ]; then
                    echo ""
                    echo "🔀 MUESTRAS CON PREDICCIONES AMBIGUAS: $ambiguous_count"
                fi
            fi
        else
            echo "    ❌ No se encontró archivo de clasificación batch"
        fi
    else
        echo "    ❌ No se encontraron archivos de clasificación"
    fi
fi
